...
The native `ios/` and `android/` folders are already committed in this
repository for reference. If you need to regenerate them from scratch (e.g.
after a clean clone or after editing `app.json`):

```bash
npx expo prebuild --clean
```

> **Warning:** `--clean` will delete any manual edits inside `ios/` and
> `android/`. See the iOS section below — you **must** re-apply the
> `ios.buildReactNativeFromSource` fix after every clean prebuild.
