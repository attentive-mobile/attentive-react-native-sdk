...
- **Expo apps using CNG (`npx expo prebuild`) must not hand-edit `MainApplication`** — prebuild regenerates it and the edit is lost. There, the Android half comes from the SDK's **Expo config plugin** instead (see Step 3b). iOS is still initialized from TypeScript.
...
1. **Is this a React Native app?** Confirm `react-native` is in the host `package.json` dependencies. The SDK requires **React Native >= 0.74**, Node >= 18, and (for iOS) Ruby >= 3.3 / CocoaPods ~> 1.16 / Xcode >= 15. For Android: **Android SDK API 26+**, **`compileSdk` 35 or higher**, and JDK 17. The `compileSdk` floor is not negotiable — the native Attentive Android SDK and the Compose runtime behind the inbox view both declare `minCompileSdk=35`, so a host on 34 fails the build outright with `requires libraries and applications that depend on it to compile against version 35 or later of the Android APIs`. Check the host's `android/build.gradle` `compileSdkVersion` before installing and tell the user if it needs raising, since that is a host-wide change and therefore their call. The wrapped Attentive Android SDK supports **API 26+** — it will build below 26 but its functionality **no-ops at runtime**, so if the host's `minSdkVersion` is below 26, warn the user that Attentive will silently do nothing for users on API 25 and below.
2. **Bare React Native or Expo?** This SDK ships native modules, so it is **not compatible with Expo Go**. A plain (bare) React Native app is fine. An Expo app must use a development build (`npx expo prebuild` / CNG). This determines the Android init path in Step 3b:
   - **Expo CNG app** (native dirs are generated — gitignored `android/`, or regenerated by prebuild): use the SDK's **Expo config plugin** for the Android init. Never hand-edit generated native files. Requires **Expo SDK 50+**.
   - **Bare RN app** (hand-maintained `android/`): edit `MainApplication` manually.
   - Expo *managed* where the user won't prebuild at all (Expo Go only): **stop and tell the user** — the SDK cannot run without native code.
...
**Expo (CNG) apps — use the config plugin, do not edit files.** Prebuild regenerates `android/`, so a manual edit is wiped. Add the plugin to `app.json` (or `app.config.js`) instead:

```json
"plugins": [
  [
    "@attentive-mobile/attentive-react-native-sdk",
    { "domain": "YOUR_ATTENTIVE_DOMAIN", "mode": "debug" }
  ]
]
```

Then run `npx expo prebuild --platform android` (or let the next `npx expo run:android` / EAS build do it). The plugin injects the same initialization shown below into the generated `MainApplication.kt`, inside `@generated` blocks it manages across re-runs. Requires **Expo SDK 50+** — on older SDKs it fails prebuild with an explicit error. The manual instructions below are for **bare React Native apps only**.
...
# Expo apps: npx expo run:android (runs prebuild automatically)
```

- **Expo CNG apps:** confirm the plugin ran by checking the generated `MainApplication.kt` for the `@generated begin attentive-react-native-sdk-init` block.
...
2. Then **emit the following block to the user, verbatim**, including every `[text](url)` link exactly as written. Do not rewrite, summarize, paraphrase, drop URLs, or convert links to plain titles. The user's terminal renders Markdown; without the `(url)` part, the links are dead.
   ...
   - [Inbox](https://github.com/attentive-mobile/attentive-react-native-sdk/blob/main/README.md#inbox) — `<AttentiveInboxView />`, a drop-in message center. Rendering it is the whole integration, but it needs a push-registered device, bounded height, and the New Architecture on iOS; its five theming props all apply on Android, and three of them (title/body/timestamp colour) apply on iOS.
   ...
   - [Marketing subscriptions](https://github.com/attentive-mobile/attentive-react-native-sdk/blob/main/README.md#marketing-subscriptions) — `optInMarketingSubscription` / `optOutMarketingSubscription`, for recording an email/SMS marketing opt-in such as a checkbox at account creation. If you serve the EU, [pixel-tracking consent](https://github.com/attentive-mobile/attentive-react-native-sdk/blob/main/README.md#pixel-tracking-consent-eu-only) covers the `trackingConsent` parameter.
   - `clearUser` and `updateDomain` are exported from the package — see the API surface in [src/index.tsx](https://github.com/attentive-mobile/attentive-react-native-sdk/blob/main/src/index.tsx) and the types in [src/eventTypes.tsx](https://github.com/attentive-mobile/attentive-react-native-sdk/blob/main/src/eventTypes.tsx).
   ```
...
- Do not hand-edit files under a generated `android/`/`ios/` in an Expo CNG app — prebuild wipes them. The Android init there comes from the config plugin only.
