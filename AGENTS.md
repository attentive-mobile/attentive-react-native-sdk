...
- Do not add `<AttentiveInboxView />` to a screen, and do not offer the inbox as a follow-up — it is **not generally available**. Its exports are tagged `@deprecated`, no stable iOS release ships inbox at all, and Android gates it behind `@RestrictTo`.
...
The base-integration guide does not wire up identify/clearUser, event recording, Creatives, marketing subscriptions, runtime domain changes, or any push setup. Step 5 already emits the linked README pointers for these to the user — see that step for the canonical block. The Inbox is skipped too, but is deliberately **not** in that block — see "Things NOT to do".
